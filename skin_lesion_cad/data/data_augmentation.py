    ...
    policy = {
        ...
        0: [[('Mixup', probability, magnitude)], [('Gaussian_noise', probability, magnitude)],
            ...
            [('Sharpness', probability, magnitude)], [('Color_casting',
                                                       probability, magnitude)], [('Equalize_YUV', probability, magnitude)],
            # [('SolarizeAdd', probability, magnitude)],
            [('Posterize', probability, magnitude)], [
            ('AutoContrast', probability, magnitude)],
            [('Solarize', probability, magnitude)], [
            ('Equalize', probability, magnitude)], [('Vignetting', probability, magnitude)],
            # shape augment
            [('Rotate', probability, magnitude)], [
            ('Flip', probability, magnitude)], [('Cutout', probability, magnitude)],
            [('Shear_x', probability, magnitude)], [
            ('Shear_y', probability, magnitude)], [('Scale', probability, magnitude)],
...
def mixup(image1, image_bg):
    ...
    if g_magnitude_is_constant:
        ...
        # magnitude is random
        factor = random.uniform(_min, _min + p * g_magnitude_value)
...
def cutout(image):
    ...
    cutout_center_height = random.randint(
        int(image_height * 0.2), int(image_height * 0.8))
    cutout_center_width = random.randint(
        int(image_width * 0.2), int(image_width * 0.8))
...
def solarize(image):
    ...
    if g_magnitude_is_constant:
        ...
        # magnitude is random
        factor = random.uniform(_max + p * g_magnitude_value, _max)
...
def shear_x(image):
    ...
    points2 = np.float32([[x_move, 0], [width + x_move, 0],
                         [width - x_move, height], [-x_move, height]])
    matrix = cv2.getPerspectiveTransform(points1, points2)
    img_shear = cv2.warpPerspective(
        image.numpy(), matrix, (width, height), borderValue=replace)
...
def shear_y(image):
    ...
    points1 = np.float32([[0, 0], [width, 0], [width, height], [0, height]])
    points2 = np.float32([[0, y_move], [width, -y_move],
                         [width, height - y_move], [0, height + y_move]])
    matrix = cv2.getPerspectiveTransform(points1, points2)
    img_shear = cv2.warpPerspective(
        image.numpy(), matrix, (width, height), borderValue=replace)
    return torch.from_numpy(np.array(img_shear, dtype=np.uint8))
...
def lens_distortion(image):
    ...
    if g_magnitude_is_constant:
        ...
        d_factor = np.random.uniform(
            _min, _min + p * g_magnitude_value, 4)      # magnitude is random
    ...
    remap = cv2.initUndistortRectifyMap(
        K, d_coef, None, M, (width, height), cv2.CV_32FC2)
    # Remap the original image to a new image
    replace = tuple(g_replace_value)
    img = cv2.remap(image.numpy(), *remap,
                    cv2.INTER_LINEAR, borderValue=replace)
...
def rotate(image):
    ...
    imgRotation = cv2.warpAffine(
        image.numpy(), matRotation, (width, height), borderValue=replace)
...
def sharpness(image):
    ...
    degenerate = degenerate.permute(
        1, 2, 0).contiguous()  # data form 3×h×w to h×w×3
...
def equalize_YUV(image):
    ...
    channels_yuv = [i for i in cv2.split(img_yuv)]
            ...
            if policy_to_select == i:
                ...
                if not have_run:
                    for func, prob, magnitude, args in tf_policy:
                        ...
                        image, run_func = _apply_func_with_prob(
                            func, image, args, prob)
                        if run_func:
...
def distort_image_with_randaugment(image, augmentation_name, num_layers, replace=None, magnitude=10):
    ...
    for i in range(num_layers):
        image, have_run_func = build_and_apply_nas_policy(
            policy[0], image, None, have_run_func)
...
def distort_image_with_modified_randaugment(image, augmentation_name, image_bg, replace=None, probability=0.7, magnitude=10):
    ...
    for i in range(len(policy)):
        image, have_run_func = build_and_apply_nas_policy(
            policy[i], image, image_bg, have_run_func)
